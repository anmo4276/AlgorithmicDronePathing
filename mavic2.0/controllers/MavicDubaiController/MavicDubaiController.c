...
int main(int argc, char **argv) {
  ...
  // Variables.
  double target_altitude = 3;  // The target altitude. Can be changed by the user.
  ...
  double waypoints[11][2] ={{191.512,-401.963},{154.055,-377.932},{169.124,-356.028}, {125.452, -333.143}, {104.749, -338.984}, {85.9754, -325.441}, {87.49, -313.417}, {62.7282, -289.790}, {66.432, -280.435}, {71.1245, -270.629}, {97.5282, -276.626}};
  ...
  while (wb_robot_step(timestep) != -1) {
    ...
    if(reachH && !reachRot[index]){
      if(wb_compass_get_values(compass)[0]<=rotX+0.007 && wb_compass_get_values(compass)[0]>=rotX-.007 && wb_compass_get_values(compass)[1]<=rotY+.007 && wb_compass_get_values(compass)[1]>=rotY-.007){
        reachRot[index]=true;
          yaw_disturbance = 0;
      }
